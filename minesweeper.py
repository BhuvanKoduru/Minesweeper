...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for s_ in self.knowledge:
            safes_=s_.known_safes()
            for each in list(safes_):
                self.mark_safe(each)
            m=s_.known_mines()
            for each in list(m):
                self.mark_mine(each)
        ...
        for s11 in self.knowledge:
            for s2 in self.knowledge:
                if s11.cells.issubset(s2.cells) and s11.cells!=s2.cells and len(s11.cells)>0 and len(s2.cells)>0:
                    new_sen=Sentence(s2.cells-s11.cells,s2.count-s11.count)
                    self.knowledge.append(new_sen)
                          
                    if(len(new_sen.cells)==new_sen.count):
                        while len(new_sen.cells)>0:
                            self.mark_mine(new_sen.cells.pop())
